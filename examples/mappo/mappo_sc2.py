...
class RunnerSC2:
    def __init__(self, config):
        ...
        self.agents = MAPPO_Agents(
            config=config, envs=self.envs
        )  # Create the Independent PPO agents.
        self.config = config
    ...
    def get_battles_info(self):
        ...
        dead_allies, dead_enemies = (
            self.envs.dead_allies_count.sum(),
            self.envs.dead_enemies_count.sum(),
        )
    ...
    def get_battles_result(self, last_battles_info):
        ...
        allies_count, enemies_count = (
            incre_battles_game * self.num_agents,
            incre_battles_game * self.num_enemies,
        )
    ...
    def test_episodes(self, test_T, n_test_runs):
        ...
        for i_test in range(n_test_runs):
            running_scores = self.agents.run_episodes(
                None, n_episodes=self.n_envs, test_mode=True
            )
            test_scores[i_test] = np.mean(running_scores)
        win_rate, allies_dead_ratio, enemies_dead_ratio = self.get_battles_result(
            last_battles_info
        )
    ...
    def run(self):
        if self.config.test_mode:
            ...
            while self.agents.current_step <= self.running_steps:
                score = self.agents.run_episodes(
                    None, n_episodes=self.n_envs, test_mode=False
                )
                ...
                if (self.agents.current_step - last_test_T) / test_interval >= 1.0:
                    ...
                    train_win_rate, allies_dead_ratio, enemies_dead_ratio = (
                        self.get_battles_result(last_battles_info)
                    )
                    results_info = {
                        "Train-Results/Win-Rate": train_win_rate,
    ...
    def time_estimate(self, start):
        ...
        sec_pass, sec_left = (
            np.mod(np.mod(time_pass, 3600), 60),
            np.mod(np.mod(time_left, 3600), 60),
        )
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = load_yaml(file_dir=f"mappo_sc2_configs/{parser.env_id}.yaml")
